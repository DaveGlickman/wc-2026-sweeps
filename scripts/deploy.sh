...
echo "============================================================"
echo
echo "NEXT (no API key needed — ESPN data is free):"
echo "  1. Trigger the first data fetch:"
echo "       gh workflow run 'Fetch World Cup data' --repo $OWNER/$REPO_NAME"
echo "  2. Run the draw, then fill config/allocations.json and config/picks.json with"
echo "     ESPN team/player ids (find team ids in public/data/matches.json), commit, push."
